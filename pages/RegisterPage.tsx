...
const RegisterPage: React.FC<RegisterPageProps> = ({ onNavigate }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const complexity = validatePassword(formData.password);
    if (!complexity.isLongEnough) {
      setError("Le mot de passe doit comporter au moins 8 caractères.");
      return;
    }
    if (!complexity.hasUpperCase || !complexity.hasLowerCase || !complexity.hasNumber || !complexity.hasSpecialChar) {
            ...
            <div>
              ...
              <div className="relative">
                <input 
                  ...
                  className="w-full px-4 py-3 bg-slate-800 border-none rounded-xl text-white text-sm font-medium focus:ring-4 focus:ring-blue-100 outline-none pr-12" 
                />
                <button
                  type="button"
              ...
              <div className="mt-2 space-y-1">
                <p className={`text-[9px] font-bold uppercase tracking-tight ${formData.password.length >= 8 ? 'text-emerald-500' : 'text-gray-400'}`}>
                  • 8 caractères minimum
                </p>
                <p className={`text-[9px] font-bold uppercase tracking-tight ${validatePassword(formData.password).hasUpperCase && validatePassword(formData.password).hasLowerCase ? 'text-emerald-500' : 'text-gray-400'}`}>
                  • Majuscules & minuscules
                </p>
                <p className={`text-[9px] font-bold uppercase tracking-tight ${validatePassword(formData.password).hasNumber && validatePassword(formData.password).hasSpecialChar ? 'text-emerald-500' : 'text-gray-400'}`}>
